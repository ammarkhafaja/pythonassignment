container_data={}# القاموس الذي سوف يتم تخزين فيه البيانات
def check_id():
...
def adding_process():
    ...
    while True:
        ...
        if "Y" in desire or "y" in desire: # في حال كان خيار الطالب y يتم استدعاء دالة اضافة طالب
            add_Student()
            desire = input("هل تريد أضافة طالب أخر؟ Y/N")
        if "N" in desire or "n" in desire:# في حال كان ادخال مالستخدم n  يتم الخروج من التابع
            return
...
def list_name_descending(dict): #تابع عرض اسماء الطلاب مرتبين بشكل تنازلي وفقاً للمعدل
    ...
    for item in sorted_list:
        result.append(item[1][0])#استخراج اسماء الطلاب من القائمة المرتبة السابقة
...
def write_to_file(dict,file_name="new_file"):#تابع كتابة محتويات القاموس في ملف علماً أن تم تعيين اسم افتراضي للملف في حال يم يقم المستخدم بتعيين الاسم
    ...
    for key in container_data.keys():# قراءة محتويات القاموس
