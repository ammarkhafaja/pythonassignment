...
def getting_loan():
    ...
    #نلاحظ ان هنالك ثلاثة أسباب مبدئية لرفض القرض لذلك تم انشاء list وسوف يخزن بها الأسباب التي منعت المصرف من اعطاء القرض للعميل
    # أي أن العميل سوف يعلم جميع أسباب الرفض
    if age <21 or age >=65:
        ban_reason.append("العمر غير مناسب")#اضافة عبارة سبب العمر الى لائحة أسباب الرفض
    if monthly_income < 3000:
        ban_reason.append("الدخل الشهري غير كاف")#اضافة عبارة سبب الدخل الشهري الغير كاف الى لائحة أسباب الرفض
    if loan_value <= 5000:
        ban_reason.append("قيمة القرض أقل من المسموح")#اضافة عبارة سبب قيمة القرض الى لائحة أسباب الرفض
    if len(ban_reason) == 0:# سوف يتم التحقق من طول القائمة  ban_reason لأنه اذا كان طول هذه القائمة لايساوي الصفر فهذا يعني وجود سبب أولي يمنع البنك من اعطاء القرض للعميل
        ...
        [print(x) for x in ban_reason]# طباعة عناصر list أسباب رفض المصرف للقرض
getting_loan()
